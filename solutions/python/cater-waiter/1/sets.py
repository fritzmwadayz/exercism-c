...
def check_drinks(drink_name, drink_ingredients):
    ...
    drink_ingredient_set = set()

    for ingredient in drink_ingredients:
        if ingredient not in drink_ingredient_set:
            drink_ingredient_set.add(ingredient)

    return f'{drink_name} Mocktail' if drink_ingredient_set.isdisjoint(ALCOHOLS) else f'{drink_name} Cocktail'
...
def tag_special_ingredients(dish):
    ...
    ingredients_to_tag = set()

    for item in ingredients:
        if item in SPECIAL_INGREDIENTS:
            ingredients_to_tag.add(item)
            
    return (dish[0], ingredients_to_tag)
...
def separate_appetizers(dishes, appetizers):
    ...
    unique = dishes.difference(appetizers)
...
def singleton_ingredients(dishes, intersection):
    ...
    counts = Counter()
    for dish in dishes:
        counts.update(dish - intersection)
